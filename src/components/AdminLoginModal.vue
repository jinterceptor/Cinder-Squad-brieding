<!-- src/components/AdminLoginModal.vue (FULL, updated: added @click.stop on backdrop) -->
<template>
  <div class="admin-modal-backdrop" @click.stop @click.self="$emit('close')">
...
<!-- src/App.vue (FULL, updated: authorize() early-return when modal open) -->
<template>
  <div
    v-if="showLogin"
    class="login-overlay"
...
export default {
  ...
  methods: {
    authorize() {
      // Prevent overlay from auto-entering site while admin modal is open
