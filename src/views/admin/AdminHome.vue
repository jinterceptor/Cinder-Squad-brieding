          ...
          <div class="table-scroll">
            <div class="table-shell">
              <div class="tr head gridFlags">
                <span class="th">Member</span><span class="th">Squad</span><span class="th">Status</span><span class="th">Warnings</span><span class="th">Notes</span>
              </div>
              <div class="rows-scroll">
                <div
...
export default {
  ...
  data() {
    return {
      ...
      // discipline
      discEndpoint: adminEndpoint(), discSecret: adminSecret(),
      discLoading: false, discSaving: false, discError: "", discOK: false, disciplineRows: [],
      // csv status (RefData Troop List + Troop Status)
  ...
  computed: {
    ...
    statusClass() {
      return (status) => ({active:'active',reserve:'reserve',eloa:'eloa',inactive:'inactive',other:'other',discharged:'discharged'}[String(status||'').toLowerCase()] || 'unknown');
    },
    ...
    promotionsTable() {
      ...
      const squadFilter = this.selectedSquad; const onlyProm = !!this.onlyPromotable;
      const rows = [];
      for (const m of (this.members||[])) {
        ...
        const rule = this.nextPromotion(m), ops = this.getOps(m);
        const nextRank = rule?.nextRank ?? null, nextAt = rule?.nextAt ?? null;
        ...
        rows.push({
          id:m.id, name:m.name||"Unknown", rank:m.rank||"N/A", squad, status,
          ops:Number.isFinite(ops)?ops:null, nextRank, nextAt, progress, opsToNext,
  ...
  methods: {
    ...
    isDischarged(status){ return String(status||"").toLowerCase()==="discharged"; },
    isInTroopList(m){
      const nk=this.nameKey(this.cleanMemberName(m?.name));
...
.windows-grid { display:grid; grid-template-columns: 320px 1fr; gap: 1rem; align-items:start; }
.left-window { position: sticky; top: 1rem; align-self:start; height: fit-content; }
.right-window { min-height: 60vh; }
.right-content { max-height: calc(100vh - 210px); overflow:auto; }

.rail { display:grid; gap:.6rem; }
.rail-card { text-align:left; border:1px dashed rgba(30,144,255,.35); background:rgba(0,10,30,.25); border-radius:.5rem; padding:.6rem; cursor:pointer; }
...
.control input, .control select, .control textarea {
  background:rgba(5,20,40,.85); border:1px solid rgba(30,144,255,.35); border-radius:.35rem; padding:.45rem .55rem; color:#e6f3ff;
}
...
.tr.head { position: sticky; top: 0; background: rgba(0, 10, 30, 0.85); backdrop-filter: blur(2px); z-index: 2; }
...
.bar { height:8px; border:1px solid rgba(30,144,255,.35); border-radius:6px; position:relative; overflow:hidden; }
...
.warn-pill { border:1px solid rgba(255,180,120,.65); background:rgba(40,10,0,.35); color:#ffd9bf; border-radius:.4rem; padding:.25rem .5rem; cursor:pointer; }
